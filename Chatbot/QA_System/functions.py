...
def trans_btoe(text,lang):
    ...
    print(type(text),text)
    '''
    url=url +'q=' + text + '&source='+lang[0:2]+'&target=en&key='+key
    print(url)
    r = requests.get(url)
    k=json.loads(r.text)
...
def speechtotxt(lang):
    print(lang)
    record()
    convert()
    transcript=transcribe_streaming(lang)
    #print("getting transcript \n"+ transcript )
    trans_btoe(transcript,lang)
    #print("finished")

def txttospeech(text,lang):
    target=lang[0:2] 
    print(target, lang)
